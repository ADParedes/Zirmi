%% Zirmi Package - Module (3-C2) Computing Cell Migration based Metrics
% Version 1.4 06-03-19
% Written By Andre Daniel Paredes | email @ andre.paredes@ymail.com
% Update:  ADP.dir_metadat
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Version 1.3 Update: ZEBRAFISH CEll kinetic Spatial Measures
%Move Excel  GTs 60, 90, 120, 150, 180 and max to excel.
clearvars -except Worthy SM Frame POI PARAMETERS ADP PhagoSight handles dataIn dataL dataR ch_GFP ch_Ph2 WoundScoreUm
disp('START: Zirmi F: Create Excel Tables')
[c1] = clock;
c1_fix = fix(c1);
%% Version 1.4 Update: Computer user define where to store metrics data
if exist('ADP.dir_metadat','var')
    %It does exist so directory is set
else
    %Please select a directory to save cell kinetic mat files and excel   
...
%          Str.ExpSet           = input('User-defined Identifier e.g. Baseline,0J,3J,9J,DoubleWound,Manuscript1 : ','s');
    ...
    case {2}
        disp('Hello User')
        Str.ExpSet           = input(strcat('Please Input Experiment Set Descriptor-no spaces',...
                                    '(E.g. Treatment1 or Dose3J)'),'s');
        Str.Fish           = input(strcat('Please Input Tg(Fish) Descriptor',...
