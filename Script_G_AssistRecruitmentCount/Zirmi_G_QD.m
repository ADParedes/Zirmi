%% Conting Leukocytes in wound region, caudal fin, or tail.  
...
input_user                  = user;
dir_mpeg                    ='G:\AndreDParedes\Paredes Data\Processed_mpeg(.mat&.tif)';
cd                          (dir_mpeg)
dir_positionFolder          = uigetdir();               % dir of Unique Experiment Processed data
BPP                         = 2^16-1;                   % MaxPixelIntensity - BPP - dependent on Bits 
boo_pc                      = ADP.boo4;
str_user                    = user;
boo_user                    = 2;                        % NOT ADPAREDES
%% Dissect Directories
[dir_Xfolder, str_positionName, ~]  = fileparts(dir_positionFolder); %break up file name 
[dir_mpeg, name, ~]                 = fileparts(dir_Xfolder); %break up file name 
temp_str_cat                        = strcat(name,'.mat');
load                                (temp_str_cat)
%%
cd                                  (dir_Xfolder);
    ...
    otherwise  %User is not ADP
        ...
        switch boo_saveCTF
            ...
            otherwise               
                ...
                dir_saveMatfiles                = dir_saveExcel;
